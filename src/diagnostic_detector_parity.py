...
def _report(name, image, detector_dim, faces):
    ...
    print(f"ndarray shape: {image.shape}")
    print(f"dtype: {image.dtype}")
    print(f"C_CONTIGUOUS: {image.flags['C_CONTIGUOUS']}")
    print(f"detector input dimensions: {detector_dim}")
    print(f"detection matrix shape: ({num_faces}, {len(faces[0]) if num_faces > 0 else 0})")
    print(f"detection count: {num_faces}")
...
def main():
    ...
    for source in args.source:
        ...
        try:
            ...
            if args.verbose_private:
                print(f"Failed to decode {source_path.name}: {e}")
            else:
    ...
    for img_info in images_info:
        if args.verbose_private:
            ...
            print(f"\n================ Image Hash: {img_info['short_hash']} ================")
